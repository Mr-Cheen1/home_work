...
func main() {
	...
	connString := "host=localhost port=8080 user=store_user password=123456 dbname=online_store sslmode=disable"
	db, err := NewDatabase(connString)
	if err != nil {
		logger.Log(fmt.Sprintf("Failed to connect to database: %v", err))
		os.Exit(1)
	}

	switch cfg.mode {
	case "server":
		server := NewServer(cfg.address, cfg.port, logger, db)
